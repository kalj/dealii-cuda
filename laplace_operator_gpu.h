...
namespace dealii
{
  ...
  private:
    ...
    MultiGpuList<Number>                  coefficient;
            ...
            const std::vector<ConstraintMatrix>         &constraints)
  {
    ...
    data.reinit (dof_handler, partitioner, constraints,
                 QGauss<1>(fe_degree+1), additional_data);

    constraint_handler.reinit(constraints,partitioner);
            ...
            const unsigned int                          level)
  {
    ...
    data.reinit (dof_handler, partitioner, ConstraintMatrix(),
                 QGauss<1>(fe_degree+1), additional_data);
  ...
  template <int dim, int fe_degree, typename Number>
  const std::shared_ptr<DiagonalMatrix<MultiGpuVector<Number>>>
